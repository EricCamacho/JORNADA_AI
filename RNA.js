...
function randomRange(min, max) {
  ...
  // Imagine esses dois números como pontos em uma linha. Se você quer encontrar um número entre esses 
  // dois pontos, a interpolação linear faz isso usando uma fórmula simples. Basicamente, ela "liga os 
  // pontos" de forma reta para pegar o valor intermediário. É muito útil quando precisamos de valore 
  // entre dois números conhecidos.
  ...
  class Neuron {
    constructor(inputs) {
      ...
      // O viés (bias) na nossa rede neural é um número que ajuda a rede a aprender. Ele permite que a
      // rede faça ajustes para entender melhor os dados. O bias é importante porque ajuda a rede a 
      //aprender padrões nos dados e a tomar decisões melhores. É como um ajuste pequeno que a rede faz
      // para se adaptar aos dados da melhor maneira possível.
...
// Neuron: A classe Neuron representa um neurônio em uma rede neural. Cada neurônio tem um viés (bias) 
// e uma lista de pesos (weightList). A função de ativação é baseada na tangente hiperbólica (Math.tanh). 
// A função g calcula a saída do neurônio com base nos sinais de entrada e a função mutate realiza 
// mutações nos pesos e no viés do neurônio com base em uma taxa fornecida.

// Definição da classe RNA (Rede Neural Artificial)
// inputCount: Número de neurônios na camada de entrada.
// levelList: Lista que especifica o número de neurônios em cada camada subsequente.
class RNA {
  ...
  // Este método realiza mutação em todos os neurônios da RNA, ajustando seus pesos de acordo com uma taxa especificada
  mutate(rate = 1) {
